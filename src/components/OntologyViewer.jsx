...
const OntologyViewer = () => {
  ...
  const pathWildcard = params['*']; // e.g., 'institutions/un/udhr'
  ...
  const pathSegments = pathWildcard.split('/');
  const id = pathSegments.pop();
  const categoryPath = pathSegments.join(' / ');
  const ontologyFile = `${pathWildcard}.n3`;
  ...
  if (loading) return <div className="container" style={{ padding: '4rem 2rem' }}>Fetching instrument semantic profile...</div>;
  if (error) return <div className="container" style={{ padding: '4rem 2rem', color: 'var(--accent-secondary)' }}>Error: {error}</div>;

  return (
    <div className="container">
      <div className="breadcrumb">
        <Link to="/">Home</Link> &gt; <span style={{ textTransform: 'capitalize' }}>{categoryPath}</span> &gt; {id}
      </div>
      ...
      <header style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <a href={`https://github.com/webcivics/ns/commits/main/public/ontologies/${ontologyFile}`} target="_blank" rel="noreferrer" className="btn">
